...
using KolikSoftware.Webeee.Net;
    ...
    public class MessageTextProcessor
    {
        ...
        public XmlDocument ProcessMessages(EeeDataSet.MessageDataTable messages, List<string> links)
        {
            using (MemoryStream stream = new MemoryStream())
            {
                ...
                foreach (XmlNode node in list)
                {
                    ...
                    conversion = SmiliesProcessor.Instance.ConvertSmilies(conversion);
        ...
        string ConvertHyperLinks(string text)
        {
            if (text.IndexOf('.') >= 0)
            {
                ...
                text = MessageTextProcessor.MailUrls.Replace(text, @"<a style=""text-decoration:none;"" xmlns="""" href=""mailto:${mail}"">[<img border=""0"" src=""Images/Link/Mail.gif"" /> ${mail}]</a>");
            }
            return text;
        }
        ...
        string EvaluateHyperlinkMatch(Match match)
        {
            ...
            if (ext == ".mp3" || ext == ".wma" || ext == ".m4a")
            {
                ...
                return @"<a style=""text-decoration:none"" xmlns="""" href=""" + link + @""" target=""_blank"">[<img border=""0"" src=""Images/Link/Media.gif"" /> " + title + "]</a>";
            }
            else
            {
                return @"<a style=""text-decoration:none"" xmlns="""" href=""" + link + @""" target=""_blank"">[<img border=""0"" src=""Images/Link/Url.gif"" /> " + server + "]</a>";
            }
        }

        string GetTemplate()
        {
            string templateName = "MessageTemplate" + "4"; //Properties.Settings.Default.MessageTemplateNo;

            if (templateName == null || templateName.Trim().Length == 0)
                throw new InvalidOperationException("Message template is not set in configuration.");

            string template = KolikSoftware.Webeee.Net.Properties.Resources.ResourceManager.GetString(templateName);
